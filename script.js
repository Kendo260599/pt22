...
function gatherInputs(){
  ...
  return {name, phone, birth, gender, dir, yearX, monthX, issueIds};
}
function saveProfile(currentResult){
  ...
  const profile = {
    ...
    input: { birth: i.birth, gender: i.gender, huongNha: i.dir, year: i.yearX, month: i.monthX, issues: i.issueIds },
    summary: {
...
function exportCSV(){
  ...
  const header = ['id','name','phone','birth','gender','huongNha','year','month','issues','cung','menh','nhom','yearWarnings','monthWarnings','createdAt'];
  ...
  try{
    ...
    orientationHandler = (e)=>{
      ...
      if(typeof e.webkitCompassHeading === 'number' && !isNaN(e.webkitCompassHeading)){
        // iOS Safari: 0° = Bắc, tăng theo chiều kim đồng hồ
        heading = e.webkitCompassHeading;
      }else if(typeof e.alpha === 'number'){
        // Nhiều trình duyệt: alpha 0° = hướng thiết bị so với Bắc (đôi khi ngược)
...
/* ========= UI: Phân tích + Lưu + Danh sách + Issues + La bàn ========= */
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  document.getElementById('profiles-tbody')?.addEventListener('click', e=>{
    ...
    if(e.target.classList.contains('view')){
      ...
      const set = new Set(p.input.issues||[]);
